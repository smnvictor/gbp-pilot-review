...
- **PR 4 — OAuth Google Business Profile** : Protocol `GoogleBusinessAdapter` + impl httpx, exceptions typées (Auth/RateLimit/5xx/Network), service OAuth (start/exchange/refresh/revoke + sync locations), routes `/oauth/google/{authorize,callback,status}` (`status` renvoie l'état de connexion + locations liées pour le dashboard client), beat task `refresh_expiring_tokens` (30 min, expire <60min). `POST /onboarding/complete` marque `clients.onboarding_completed_at` (et `business_name` optionnel depuis la location choisie).
- **PR 5 — Polling** : `PollingService.poll_client` (dedup par `google_review_id`), Celery `dispatch_pollings` (beat fixe 4×/jour à 11h/14h/17h/20h Europe/Paris, non configurable côté client) + `poll_client_reviews` (queue=polling, retry 3x backoff 1800s) + `process_review_task`.
...
- **PR 7 — Generation Claude** : Protocol `LLMProvider` + impl Anthropic SDK avec **prompt caching** (cache_control ephemeral) et tool-use schema strict, 13 codes `details` validés via `Literal`, modèle `claude-sonnet-4-6`, routage post-génération (status=0 → team review, status=1 → suggestion/team selon `validation_mode`). `POST /test/generate-response` (dry-run depuis le dashboard : avis fictif `{review_content, rating}` ou réel `{review_id}`, aucun quota consommé ni écriture DB).
